...
async def crear_transferencia(
    ...
    if not row:
        raise HTTPException(status_code=500, detail="Error al recuperar la transferencia creada.")
...
async def obtener_transferencia(
    ...
    row = await get_transferencia(db, idtransferencia)
    if not row:
        raise HTTPException(status_code=404, detail=f"Transferencia {idtransferencia} no encontrada.")
    return _to_response(row)
...
async def listar_por_almacen(
    ...
    estatus:      Optional[str] = Query(default=None,
                                        description="pendiente | confirmada | rechazada"),
    db: AsyncSession = Depends(get_db),
):
    """
